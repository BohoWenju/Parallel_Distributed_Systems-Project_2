...
double extended_kNN(node_t* root,double* Y_i,int index_y,int d,int k,
  ...
  if (root!=NULL)
    temp_dist=distance_calc1(root->vector,Y_i+index_y*d,d);
  for (int i=0; i<k; i++){
  ...
  if ((temp_dist>=root->mu)&&(root->B_index_r!=-2)){
    ...
    if(root->B_size_l>0)
      ...
      // get max distance
    else if (root->B_index_l==-1)
      max_dist=extended_kNN(root->left_child,Y_i,index_y,d,k,k1,B_vector,B_indexes);
    if ((fabs(temp_dist-max_dist)>=root->mu)&&(root->B_index_r==-1))
      max_dist=extended_kNN(root->right_child,Y_i,index_y,d,k,k1,B_vector,B_indexes);
    else if ((fabs(temp_dist-max_dist)>=root->mu)&&(root->B_index_r>=0)){
      for (int i=0; i<root->B_size_r; i++){
        double dist=distance_calc1(B_vector+(root->B_index_r)*d+i*d,Y_i+index_y*d,d);
        for (int j=0; j<k; j++)
            if (dist<k1->ndist[j+index_y*k]){
...
void fill_send_Arrays(node_t* root,int** index,double** dim,
  ...
  if (root_flag==true){
    ...
    if ((root->B_index_r!=-2)&&(root->B_size_r>0)){
      size_branches[b_size]=root->B_size_r;
      (b_size)++;
...
void empty_recv_Arrays(node_t** root,int* index,double* dim,
  ...
  int* size_branches,bool root_flag,int m){
  ...
  if (root_flag==true){
    ...
    if ((index[index_t+1]!=-1)&&(flag1)){
        (*root)->B_index_l=-1;
        (*root)->left_child=(node_t*)malloc(sizeof(node_t));
        (*root)->left_child->vector=(double*)malloc(d*sizeof(double));
        empty_recv_Arrays(&((*root)->left_child),index,dim,mu,B_holder,B_indexes,1,d,level+1,size_branches,false,m);
    }
    if ((flag2)&&(index[index_t+2]!=-1)){
        (*root)->B_index_r=-1;
        (*root)->right_child=(node_t*)malloc(sizeof(node_t));
        (*root)->right_child->vector=(double*)malloc(d*sizeof(double));
        empty_recv_Arrays(&((*root)->right_child),index,dim,mu,B_holder,B_indexes,2,d,level+1,size_branches,false,m);
    }
  }
  else {
    (*root)->index_p=index[index_t];
    ...
    if ((flag1)&&(index[n_index]!=-1)){
        ...
        (*root)->left_child=(node_t*)malloc(sizeof(node_t));
        (*root)->left_child->vector=(double*)malloc(d*sizeof(double));
        empty_recv_Arrays(&((*root)->left_child),index,dim,mu,B_holder,B_indexes,n_index,d,level+1,size_branches,false,m);
    }
    if ((index[n_index+1]!=-1)&&(flag2)){
        (*root)->B_index_r=-1;
        (*root)->right_child=(node_t*)malloc(sizeof(node_t));
        (*root)->right_child->vector=(double*)malloc(d*sizeof(double));
        empty_recv_Arrays(&((*root)->right_child),index,dim,mu,B_holder,B_indexes,n_index+1,d,level+1,size_branches,false,m);
    }
  }
}
...
knnresult distrAllkNN_extended(double*  X,int n, int d, int k){
  ...
  for (int i=1; i<size; i++){
    ...
    int mpi_size=sendcount[(rank-1+size)%size]/d;
    node_t* root2=(node_t*)malloc(sizeof(node_t));
    root2->vector=(double*)malloc(d*sizeof(double));
    // recreate the tree
    empty_recv_Arrays(&root2,index_snd,dim_snd,mu_snd,B_holder,B_indexes,0,d,0,size_branches_n,true,mpi_size);
    root1=root2;
